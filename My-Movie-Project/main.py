...
#MODELS
from Models.model import User, Movie_model
#PYTHON
...
@app.get('/movies', tags=['Movies'], response_model=List[Movie_model], status_code=status.HTTP_200_OK, dependencies=[Depends(JWTBearer())]) #Le indicamos que el modelo de respuesta sera una lista de Movies
def get_movies():
    try:
...
#Mediante parametros
@app.get('/movies/{movie_id}', tags=['Movies'], response_model=Movie_model, status_code=status.HTTP_200_OK) #dentro de las llaves le especificamos que parametro debe ingresar. Le indicamos que el modelo de respuesta sera de la clase Movie
...
@app.get('/movies/category/', tags=['Movies'], response_model=List[Movie_model],status_code=status.HTTP_200_OK) #le agregamos el slash a la ruta para que detecte que se agregara un parametro query y no nos remplace el otro metodo creado anteriormente.
...
def create_movie(movie: Movie_model): #requerimos una pelicula de la clase Movie
    try:
...
def update_movie(id_movie:int, movie : Movie_model):
    for items in movies:
